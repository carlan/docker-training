...
Later on, virtualization was the thing. I'm refering to the virtualization as a method of divide the system resources between different applications. To use this virtualization we need a Virtual Machine (VM now on). The simplest definition of a VM is: A VM is an emulation of a physical computing environment.
